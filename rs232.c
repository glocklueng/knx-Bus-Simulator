...
int RS232_PrepComport(int comport_number, int baudrate, const char *mode) {
	return(0);
}
...
int RS232_SendBytes(int comport_number, unsigned char byte1, unsigned char byte2)
{
  int n;

  n = write(Cport[comport_number], &byte1, 1);
  n = write(Cport[comport_number], &byte2, 1);
  if(n<0)  return(1);
